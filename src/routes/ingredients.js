...
router.get('/', async (req, res) => {
  try {
    const { rows } = await query('SELECT * FROM ingredients ORDER BY created_at');
...
router.post('/', async (req, res) => {
  try {
    ...
    const { rows } = await query(
      'INSERT INTO ingredients (name, icon) VALUES ($1, $2) RETURNING id',
      [name, icon || 'fa-circle']
    );
...
router.put('/:id', async (req, res) => {
  try {
    const { name, icon } = req.body;
    const { id } = req.params;
    await query('UPDATE ingredients SET name = $1, icon = $2 WHERE id = $3', [name, icon, id]);
